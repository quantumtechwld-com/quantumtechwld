...
const metadataByLocale: Record<Locale, { title: string; description: string }> = {
  pt: {
    title: "Desenvolvimento de Sistemas sob Medida, CRM, ERP e SaaS | QuantumTech",
    description:
      "Desenvolvemos sistemas sob medida, portais B2B, CRMs, ERPs e plataformas SaaS com stack moderna, modelagem de processo e UX que equipes adotam de verdade.",
  },
  en: {
    title: "Custom Software Development, CRM, ERP and SaaS Platforms | QuantumTech",
    description:
      "We build custom systems, B2B portals, CRMs, ERPs and SaaS platforms with a modern stack, process modeling and UX that teams actually adopt.",
  },
  es: {
    title: "Desarrollo de Sistemas a Medida, CRM, ERP y Plataformas SaaS | QuantumTech",
    description:
      "Desarrollamos sistemas a medida, portales B2B, CRMs, ERPs y plataformas SaaS con stack moderno, modelado de proceso y UX que los equipos adoptan de verdad.",
  },
};
...
export async function generateMetadata({
  ...
  const path = withLocalePrefix(currentLocale, "/solutions/sistemas-sob-medida");
  ...
  return {
    ...
    alternates: {
      ...
      languages: {
        pt: `${BASE_URL}/solutions/sistemas-sob-medida`,
        en: `${BASE_URL}/en/solutions/sistemas-sob-medida`,
        es: `${BASE_URL}/es/solutions/sistemas-sob-medida`,
        "x-default": `${BASE_URL}/solutions/sistemas-sob-medida`,
      },
    },
    openGraph: {
      title: metadata.title,
...
export default async function SistemasSobMedidaSolutionPage({
  params,
}: Readonly<{
  params: Promise<{ locale: string }>;
}>) {
  ...
  const useCases = [1, 2, 3, 4].map((index) => ({
  ...
  const deliverableIcons = [Database, Layers] as const;
  const useCaseIcons = [LineChart, Users, Settings, Rocket] as const;
  ...
  return (
    <main className="bg-[#050816] text-white">
      ...
      <section className="relative overflow-hidden border-b border-white/8 bg-linear-to-b from-[#08101f] via-[#050816] to-[#050816]">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(99,102,241,0.12),transparent_30%),radial-gradient(circle_at_top_right,rgba(139,92,246,0.15),transparent_28%)]" />
            ...
            <div className="grid gap-5">
              {(() => {
                return deliverables.map((item, index) => {
                  const Icon = deliverableIcons[index] ?? Database;
                  return (
      ...
      <section className="mx-auto max-w-6xl px-6 py-20">
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
          {useCases.map((item, index) => {
            const Icon = useCaseIcons[index] ?? Rocket;
            return (
